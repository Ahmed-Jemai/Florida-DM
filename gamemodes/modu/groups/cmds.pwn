...
CMD:groupmembers(playerid,params[])
{
	...
	for(new i=0; i < MAX_PLAYERS; i++)
	{
	    ...
	    {
	        format(str,sizeof(str),"{FFFFFF}Online Member: %s\tID:%d\tRank:%s\n",ReturnName(i),i,PlayerInfo[i][grouprank]);
			strcat(string, str);
	    }
	}
...
CMD:wtlock(playerid,params[])
{
	...
	if(GroupInfo[id][gchat] == 0)
	{
		...
		format(str,sizeof(str),"[%s]: %s has locked the WT.",GroupInfo[id][gName],ReturnName(playerid));
	...
	if(GroupInfo[id][gchat] == 1)
	{
		...
		format(str,sizeof(str),"[%s]: %s has unlocked the WT.",GroupInfo[id][gName],ReturnName(playerid));
		SendGroupMessage(color_wt, id, str);
	    return 1;
	}
	return 1;
...
//	if(strlen(str) > 1 || strlen(str) < 5 ) return SendClientMessage(playerid,color_red,"[Error]{FFFFFF}: Your group tag must be a maxinum 5 characters.");
